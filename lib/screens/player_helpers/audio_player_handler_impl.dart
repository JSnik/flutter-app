...
class AudioPlayerHandlerImpl extends BaseAudioHandler with SeekHandler implements AudioPlayerHandler {
  ...
  AudioPlayerHandlerImpl(PlayerProvider pProvider, BuildContext context) {
  ...
  Future<void> _init(BuildContext context) async {
    ...
    await updateQueue(_playerProvider.mediaLibrary.baseItems[MediaLibrary.titleIdRadio]!);
    ...
    switch (parentMediaId) {
      case AudioService.recentRootId:
        ...
        return _recentSubject.hasValue
            ...
            : stream.shareValue();
      default:
        return Stream.value(_playerProvider.mediaLibrary.baseItems[parentMediaId])
  ...
  Future<void> updateMediaItem(MediaItem mediaItem) async {
    ...
    if (_previousPlayNowMediaItem.id == '' || _previousPlayNowMediaItem.album != mediaItem.album) {
      _previousPlayNowMediaItem = mediaItem;
      needUpdate = true;
    }
    if (_player.sequence!= null && _player.sequence!.isNotEmpty && needUpdate == true) {
      final index = queue.value.indexWhere((item) => item.id == mediaItem.id);
      if (index >= 0) {
        // _mediaItemExpando[_player.sequence![index]] = mediaItem;
        updateCurrentMediaItem(mediaItem);
        _playerProvider.indexOfPlayingItemBeforeInternetOff = index;
      }
    }
  }
  ...
  Future<void> skipToNext() async {
    if (mediaItem.value != null && mediaItem.value!.genre != null && mediaItem.value!.genre == 'MainData') {
      _playerProvider.carouselSliderControllerAppBar.nextPage();
    }
    await _player.seekToNext();
    _playerProvider.carPlayModule.updatePlayingStatusOnListItems(mediaItem.value!.title);
  }

  @override
  Future<void> skipToPrevious() async {
    if (mediaItem.value != null && mediaItem.value!.genre != null && mediaItem.value!.genre == 'MainData') {
      _playerProvider.carouselSliderControllerAppBar.previousPage();
    }
