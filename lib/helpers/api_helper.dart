...
class ApiHelper {
  ...
  Future<http.Response> getWithoutBaseUrl(String url, Map<String, String>? headers) async {
    try {
  ...
  Future<http.Response> getRequestWithToken({required String url, Map<String, String>? headers, required  BuildContext context}) async {
    ...
    if (resp.statusCode == 403 || resp.statusCode == 401) {
      ...
      return await getRequestWithToken(url: url, headers: headers, context: context);
    } else {
      return resp;
    }
  }

  Future<http.Response> postRequestWithToken({required String url, required String body, Map<String, String>? headers, required BuildContext context}) async {
   String token = Singleton.instance.getTokenFromSharedPreferences();
    Map<String, String> baseHeader = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $token',
  ...
  Future<bool> refreshToken(BuildContext context) async {
    ...
    String refreshToken = Singleton.instance.getRefreshTokenFromSharedPreferences();
    ...
    if (refreshToken != '') {
      ...
      if (json.decode(response.body) != null && json.decode(response.body)['data'] != null && json.decode(response.body)['data']['accessToken'] != null
  ...
  http.Response _returnResponse(http.Response response) {
    ...
    switch (response.statusCode) {
      case 200:
        ...
        return response;
      case 400:
        Singleton.instance.showErrorMassage(Singleton.instance.translate('error'), Singleton.instance.translate('invalid_request'));
        return defResp;
      case 401:
