...
class PodcastsProvider with ChangeNotifier {
  ...
  Podcasts audioPodcasts = Podcasts(apiVersion: '', filters: Filters(sort: [], categories: [], subCategories: []), data: [], banners: []);
  Podcasts videoPodcasts = Podcasts(apiVersion: '', filters: Filters(sort: [], categories: [], subCategories: []), data: [], banners: []);
  interview.Interview interviewPodcasts = interview.Interview(apiVersion: '', filters: interview.Filters(sort: [], categories: []), data: [], banners: []);
  Filters filtersAudio = Filters(sort: [], categories: [], subCategories: []);
  Filters filtersVideo = Filters(sort: [], categories: [], subCategories: []);
  interview.Filters filtersInterview = interview.Filters(sort: [], categories: []);

  int currentAppBarIndex = 0;

  Future<void> getAllPodcasts(BuildContext context, {required bool isFromInit}) async {
    getAudioPodcasts(context, isFromInit);
    getVideoPodcasts(context, isFromInit);
    getInterviewPodcasts(context, isFromInit);
  }

  Future<void> getAudioPodcasts(BuildContext context, bool isFromInit) async {
    getAudioPodcastsResponseState = ResponseState.stateLoading;
    if (isFromInit == false) {notifyListeners();}
    ApiHelper helper = ApiHelper();
    String languageCode = Singleton.instance.getLanguageCodeFromSharedPreferences();
    Sort sortElem = Sort(name: 'Test', sortBy: 'dateFrom', sortOrder: 'desc'); 'dateFrom';
    if (filtersAudio.sort.isNotEmpty) {
      sortElem = filtersAudio.sort.firstWhere((element) => element.isSelected);
    }

    String apiKey = '/api/podcasts/audio/$languageCode/${sortElem.sortBy}/${sortElem.sortOrder}';
  ...
  Future<void> getVideoPodcasts(BuildContext context, bool isFromInit) async {
    ...
    String apiKey = '/api/podcasts/video/$languageCode/${sortElem.sortBy}/${sortElem.sortOrder}';
  ...
  Future<void> getInterviewPodcasts(BuildContext context, bool isFromInit) async {
    getInterviewPodcastsResponseState = ResponseState.stateLoading;
    if (isFromInit == false) {notifyListeners();}
    ApiHelper helper = ApiHelper();
    String languageCode = Singleton.instance.getLanguageCodeFromSharedPreferences();
    interview.Sort sortElem = interview.Sort(name: 'Test', sortBy: 'dateFrom', sortOrder: 'desc'); 'dateFrom';
    if (filtersInterview.sort.isNotEmpty) {
      sortElem = filtersInterview.sort.firstWhere((element) => element.isSelected);
    }

    String apiKey = '/api/interviews/$languageCode/${sortElem.sortBy}/${sortElem.sortOrder}';
  ...
  bool isFiltersHasChanges() {
    for (var i = 0; i < audioPodcasts.filters.sort.length; i++) {
      if (audioPodcasts.filters.sort[i].isSelected != filtersAudio.sort[i].isSelected) {
        return true;
      }
    }
  ...
  void setCurrentSortFilter(int sortIndex) {
    if (currentAppBarIndex == 0) {
      for (var element in filtersAudio.sort) {element.isSelected = false;}
      filtersAudio.sort[sortIndex].isSelected = true;
    }
    if (currentAppBarIndex == 1) {
      for (var element in filtersVideo.sort) {element.isSelected = false;}
      filtersVideo.sort[sortIndex].isSelected = true;
    }
    if (currentAppBarIndex == 2) {
      for (var element in filtersInterview.sort) {element.isSelected = false;}
