...
class PlayerProvider with ChangeNotifier {
  ...
  Future<bool> initPlayerHandle(BuildContext context) async {
    if (isPlayerHandleInit == false) {
      Singleton.instance.audioHandler = await AudioService.init(
        builder: () => AudioPlayerHandlerImpl(this, context),
  ...
  void setNoInternetItems() async {
    ...
    radioItemsToNoInternet = mediaLibrary.baseItems[MediaLibrary.titleIdRadio]?? [];
    podcastsItemsToNoInternet = mediaLibrary.baseItems[MediaLibrary.titleIdPodcasts]?? [];
    interviewItemsToNoInternet = mediaLibrary.baseItems[MediaLibrary.titleIdInterview]?? [];
    favoritesItemsToNoInternet = mediaLibrary.baseItems[MediaLibrary.titleIdFavorites]?? [];
    ...
    await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdRadio]!);
    await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdPodcasts]!);
    await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdInterview]!);
    await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdFavorites]!);
  ...
  void setInternetIsAvailableItems(BuildContext context) async {
    ...
    if (radioItemsToNoInternet.isNotEmpty) {
      mediaLibrary.baseItems[MediaLibrary.titleIdRadio] = radioItemsToNoInternet;
      mediaLibrary.baseItems[MediaLibrary.titleIdPodcasts] = podcastsItemsToNoInternet;
      mediaLibrary.baseItems[MediaLibrary.titleIdInterview] = interviewItemsToNoInternet;
      mediaLibrary.baseItems[MediaLibrary.titleIdFavorites] = favoritesItemsToNoInternet;
      await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdRadio]!);
      await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdPodcasts]!);
      await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdInterview]!);
      await Singleton.instance.audioHandler.updateQueue(mediaLibrary.baseItems[MediaLibrary.titleIdFavorites]!);
    ...
    if (_itemsBeforeInternetOff.isNotEmpty) {
      ...
      if (_internetOffPlayingState == true) {
        playAllTypeMedia(_itemsBeforeInternetOff, indexOfPlayingItemBeforeInternetOff, '', '');
      }
    }
  }
  ...
  void initTimer() {
    if (timerIsInit == false) {
      timerIsInit = true;
      _playNowUpdateTimer = Timer.periodic(const Duration(seconds: 5), (Timer t) => _playNowTimer());
    }
  }
  ...
  void _playNowTimer() {
    if (_audioPlayer.playing && _currentPlayNowUrl != '' && itIsStream == true) {
  ...
  Future<void> updateAndroidAutoAndCarPlayItems(List<dynamic> items) async {
   // await initPlayerHandle(context).then((value) async {
      if (items.isNotEmpty) {
            ...
            if (aP.podcasts != null && aP.podcasts!.isNotEmpty) {
              ...
              for (Podcast p in aP.podcasts!) {
                MediaItem pItem = MediaItem(
                    playable: false,
                    id: p.id.toString(),
                    artUri: Uri.parse(apiBaseUrl + p.image),
                    title: p.title);
                ...
                if (p.episodes.isNotEmpty) {
                  for (Episode e in p.episodes) {
                    ...
                    MediaItem item = MediaItem(
                        ...
                        artUri: Uri.parse(apiBaseUrl + e.contentData.cards.first.image),
                        duration: Duration(seconds: e.contentData.cards.first.audioDuration),
                        artist: p.title,
                        title: e.title);
        ...
        if (items.first is InterviewData && items.first.contentData != null && items.first.contentData.cards != null && items.first.contentData.cards.isNotEmpty) {
          ...
          for (InterviewData e in items) {
            if (e.contentData.cards.isNotEmpty && e.contentData.cards.first.audioFile != '') {
              MediaItem item = MediaItem(
                  playable: true,
                  duration: Duration(seconds: e.contentData.cards.first.audioDuration!),
                  id: apiBaseUrl +  e.contentData.cards.first.audioFile!,
          ...
          mediaLibrary.baseItems[MediaLibrary.titleIdInterview] = firstLayerEpisodeList;
        ...
        if (items.first is MainData) {
          ...
          for (MainData mD in items) {
            MediaItem item = MediaItem(
                ...
                genre: 'MainData',
                //artist: 'Vasia',
                artUri: Uri.parse(apiBaseUrl + mD.upperImage),
                title: mD.name);
  ...
  void playAllTypeMedia(List<dynamic> items, int indexToPlay, String title, String description) {
    if (items.isNotEmpty) {
      if (items.first is MediaItem) {
        _playOrSeekForTypeMediaItem(items as List<MediaItem>, indexToPlay, title);
      } else {
        _playOrSeek(items, indexToPlay, title, description);
      }
    }
    //checkMiniPlayerStatus();
  }

  void _playOrSeekForTypeMediaItem(List<MediaItem> items, int indexToPlay, String title) async {
    if (Singleton.instance.audioHandler.queue.value.first.id == items.first.id) {
  ...
  void _playOrSeek(List<dynamic> items, int indexToPlay, String title, String description) async {
    var indexInQueue = await _isMediaCurrentlyInQueue(items, indexToPlay);
    if (indexInQueue != -1) {
...
////////Episode////////
      if (items.first is Episode && items.first.contentData != null && items.first.contentData.cards != null && items.first.contentData.cards.isNotEmpty) {
        ...
        for (Episode e in items) {
          MediaItem item = MediaItem(
              ...
              displayDescription: description == '' ? e.description : description,
              title: e.title);
...
///////InterviewData///////
  ...
  Future<int> _isMediaCurrentlyInQueue(List<dynamic> listOfMedia, int indexToPlay) async {
    ...
    for (var media in listOfMedia) {
      if (media is MainData) {
        index = Singleton.instance.audioHandler.queue.value.indexWhere((item) => item.id == media.streamLink);
        if (index != -1) {
  ...
  void _carPlayPlay ({required ContentType type, required String name}) async {
    if (type == ContentType.mainRadio) {
      ...
      carouselSliderControllerAppBar.jumpToPage(indexToPlay);

      //playAllTypeMedia([mainScreenProvider.mainScreenData.data[mainScreenProvider.currentSelectedDataIndex]], 0, context);
    ...
    if (type == ContentType.podcast) {
      ...
      playAllTypeMedia(podcastsToPlay, indexToPlay, podcastsToPlay[indexToPlay].title, '');
    }
    if (type == ContentType.interview) {
      List<MediaItem> interviewList = mediaLibrary.baseItems[MediaLibrary.titleIdInterview]!;
      int indexToPlay = 0;
...
/////////Car Play///////////
  ...
  void _updateCarPlayAudioPodcastsList(List<AudioPodcast> listToUpdate)  {
    if (Platform.isIOS) {
      carPlayModule.updateAudioPodcastsList(listToUpdate, onPodcastsTap: (String name) {
        _carPlayPlay(type: ContentType.podcast, name: name);
      });
    }
  }

  void _updateCarPlayInterviewList(List<MediaItem> listToUpdate, String nameToPlay)  {
    if (Platform.isIOS) {
      carPlayModule.updateInterviewList(listToUpdate, onInterviewItemTap: (String name) {
