...
class CarPlayModule {
  ...
  CPConnectionStatusTypes _carPlayConnectionStatus = CPConnectionStatusTypes.unknown;
        ...
        rootTemplate: CPTabBarTemplate(
          templates: [
            CPListTemplate(
              ...
              emptyViewTitleVariants: [Singleton.instance.translate('loading_car_play')],
  ...
  void updatePlayingStatusOnListItems(String name) async {
  ...
  void updateRadioList(List<MediaItem> mainDataList, int indexToPlay, {required final Function(String) onRadioItemTap}) async {
    if (Platform.isIOS) {
      var currentRootTemplate = FlutterCarplay.rootTemplate!;
      ...
      for (MediaItem mD in mainDataList) {
        ...
        String imageUrl = 'http:/skonto2.mediaresearch.lv/uploads/LOGO/lounge.png';
        if (mD.artUri != null && _isImageLink(mD.artUri.toString())) {
            imageUrl =  mD.artUri.toString().contains('http') ? mD.artUri.toString() : apiBaseUrl + mD.artUri.toString();
        }
        var item = CPListItem(
          text: mD.title,
  ...
  void updateAudioPodcastsList(List<AudioPodcast> audioPodcastsList, {required final Function(String name) onPodcastsTap}) {
    if (Platform.isIOS) {
      var currentRootTemplate = FlutterCarplay.rootTemplate!;
      ...
      for (AudioPodcast aP in audioPodcastsList) {
        ...
        if (aP.podcasts != null && aP.podcasts!.isNotEmpty) {
          for (var podcast in aP.podcasts!) {
            String podcastImageUrl = 'https://farm4.staticflickr.com/3224/3081748027_0ee3d59fea_z_d.jpg';
            if (podcast.image != '' && _isImageLink(podcast.image)) {
              podcastImageUrl = podcast.image.contains('http') ? podcast.image : apiBaseUrl + podcast.image;
            }
            var item = CPListItem(
              text: podcast.title,
              ...
              onPress: (complete, self) {
                if(_audioFiles[self.uniqueId] != null) {
                  ...
                  openListOfEpisodes(_audioFiles[self.uniqueId]!, onPodcastsItemTap: (String name) {
                   // onPodcastsTap(name);
                  });
                }
              },
            ...
            if (podcast.episodes.isNotEmpty) {
              ...
              for (var episode in podcast.episodes) {
                String imageUrl = 'https://farm4.staticflickr.com/3224/3081748027_0ee3d59fea_z_d.jpg';
                if (episode.contentData.cards.first.image != null && _isImageLink(episode.contentData.cards.first.image)) {
                  imageUrl = episode.contentData.cards.first.image.contains('http') ? episode.contentData.cards.first.image : apiBaseUrl +  episode.contentData.cards.first.image;
                }
                mediaFilesList.add(
                    CPListItem(
                      text: episode.title,
                      ...
                      onPress: (complete, self) {
                        ...
                        if(_audioFiles[_lastOpenPodcastId] != null) {
                          openListOfEpisodes(_audioFiles[_lastOpenPodcastId]!, onPodcastsItemTap: (String name) {
                            // onPodcastsTap(name);
                          });
                        }
                      },
        ...
        _audioPodcastsListISections.add(
            CPListSection(
                header: aP.name,
                items: podcastsList
            )
  ...
  void openListOfEpisodes(List<CPListItem> list, {required final Function(String name) onPodcastsItemTap}) {
    if (Platform.isIOS) {
      FlutterCarplay.push(
  ...
  void updateInterviewList(List<MediaItem> interviewList, {required final Function(String name) onInterviewItemTap}) {
    if (Platform.isIOS) {
      var currentRootTemplate = FlutterCarplay.rootTemplate!;
      ...
      for (MediaItem interview in interviewList) {
        String interviewImageUrl = 'https://farm4.staticflickr.com/3224/3081748027_0ee3d59fea_z_d.jpg';
        if (interview.artUri != null && interview.artUri.toString() != '' && _isImageLink(interview.artUri.toString())) {
          interviewImageUrl = interview.artUri.toString().contains('http') ? interview.artUri.toString() : apiBaseUrl +  interview.artUri.toString();
        }
        itemsList.add(
            CPListItem(
        ...
        rootTemplate: CPTabBarTemplate(
          templates: [
            CPListTemplate(
              ...
              emptyViewTitleVariants: [Singleton.instance.translate('no_internet_connection')],
              title: Singleton.instance.translate('radio'),
              showsTabBadge: false,
              systemIcon: "house.fill",
            ),
